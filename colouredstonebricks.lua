...
for _, row in ipairs(colours) do
	...
	minetest.register_node("vanilla_plus:colouredstonebricks"..name, {
	...
	if minetest.get_modpath("vanilla_plus:colouredstonebricks") then
	    stairsplus:register_all("vanilla_plus:colouredstonebricks", name, "colouredstonebricks:"..name, {
	        description = description.." Stone Brick",
			tiles = {"colouredstonebricks_"..name..".png"},
			groups = {cracky=3},
			sounds = default.node_sound_stone_defaults(),
